...
class runInfo:
    def __init__(
        self, domain, problem, solver, res_path, resolution, melements="C Lagrange 1"
    ):
        self._domain = domain
    ...
    def get_previous_run_num(self):
        ...
        if mpiRank == 0:
            self._execute_command(
                "cp " + file_name + " " + self._file_name + "_back_up.txt"
            )
    ...
    def _gather_run_num(self):
        ...
        if dolfin.__version__ <= "1.6.0":
            ...
            v_run_num = PETScVector(MPI.comm_self)
    ...
    def _get_solver_info(self):
        ...
        if solver.__class__.__name__ == "mNewtonSolver":
            m = "    <td>"
            m += (
                "Newton, omega =%g" % (solver.get_omega())
                + " <br> "
                + solver.get_name()
            )
            return m + s_init + "</td>\n"
    ...
    def export_run(self, test_mode=False):
        ...
        self._execute_command(
            "cp " + file_name + " " + self._file_name + "_back_up.html"
        )
