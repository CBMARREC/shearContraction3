from .ProblemCs import mNonlinearVariationalProblem, mEnergyMinimizationProblem
import warnings
...
from .ncg_parallel.ncg_PETS import mpi_comm_world
import time
...
class mSolver:
    def __init__(self, problem, res_path="results/", save_run=True):
        ...
        self._F11 = None
        self._norm_u, self._norm_u_def, self._meshy, self._V_norm_u = (
            None,
            None,
            None,
            None,
    ...
    def add_disturbances(self, ampl=1e-2):
        comm = mpi_comm_world()
        ...
        N = u_array.shape[0]
        u_array = u_array + ampl * np.random.rand(N)
    ...
    def initialization_ncg(self, psi_name):
        ...
        problem = mEnergyMinimizationProblem(
            V, psi_name, domain, self._problem.get_W, el_order=self._problem.el_order()
        )
        solver_t = "fixed_centers"  #'free_centers' #
        solver = m_ncgSolver(problem, res_path="", solver=solver_t, save_run=False)
        u_init = solver.solve()
            ...
            def eval(self, value, x):
                ...
                if x[1] < a:
                    ...
                    value[1] = (
                        a * (1 - k22)
                        - (1 - k22) * x[1]
                        + 0.0 * np.sin(10 * np.pi * x[1]) * x[0] * (1 - x[0])
                    )
                # k11, k12, k21, k22 = 2, 2, 2, 2
                value[0] = (l1 - 1) * x[0]
    ...
    def init_fromHDF5File(
        self, exp_num, folder_path="/home/ggrekas/Documents/mClasses/results/vtkFiles/"
    ):
    ...
    def compute_funcs(self, exp_info=None):
        ...
        self._dens, self._dens_undef, self._V_dens = calculate_dens(
            self._u, J, el_order, V0, mesh, V
        )
        self._norm_u, self._norm_u_def, self._meshy, self._V_norm_u = calculate_u_norms(
            self._u, J, el_order, V0, mesh, V
        )
    ...
    def _make_functions_plot(self, exp_info, window_width, window_height):
        ...
        det_plot = plot(
            self._detF,
            title="detF for experiment " + exp_info,
            window_width=window_width,
            window_height=window_height,
        )
        name = plotsPath + exp_info + "_detF"
        det_plot.elevate(50)
        # det_plot.update(detF);
        det_plot.write_png(name)

        F22_plot = plot(
            self._F22,
            title="F22 for experiment " + exp_info,
            window_width=window_width,
            window_height=window_height,
        )
        ...
        dens_plot = plot(
            self._dens,
            title="density on deformed configuration" + exp_info,
            window_width=window_width,
            window_height=window_height,
        )
    ...
    def _save_functions(self, exp_info):
        ...
        self._detF.rename("detF", "detF")
        file_J << self._detF
        self._dens.rename("dens", "dens")
        file_dens << self._dens
        self._u.rename("u", "u")
        file_u << self._u
    ...
    def save_all_function_info(self, exp_info=None):
        ...
        try:
            ...
            if self._mpiRank == 0:
                np.save(
                    results_path + "energy_i",
                    self._energy_i[0 : np.int(self._niter / self._print_Niter) + 1],
                )
        except:
            print(
                "Energy value per 100 iterations is available only for the non "
    ...
    def norm_extract_vertex_values(self, exp_info=None):
        ...
        np.save(results_path + str(mpiRank) + "u_norm_def_v", u_norm_def_vert)

        np.save(results_path + str(mpiRank) + "coor_v", dof_coordinates)
        np.save(results_path + str(mpiRank) + "coor_def_v", coor_def)
    ...
    def extract_dens_cell_values(self, exp_info=None):
        ...
        np.save(results_path + str(mpiRank) + "c_centers_undef", dof_coordinates)
    ...
    def _iteration_plots(self, F11, F22, exp_info):
        ...
        self._plot(
            ...
            "y=0",
            "F22",
            "F22 (def. gradient element) on axis x = 0",
            exp_info + "_F22_x=0",
        )
        ...
        self._plot(
            ...
            "y=0",
            "F11",
            "F11 (def. gradient element) on axis x = 0",
            exp_info + "_F11_y=0",
        )
...
class mNewtonSolver(mSolver):
    ...
    def adaptive(self, omegaMin=0.005):
        ...
        while eps > tol and niter < maxiter:
            ...
            omega, niter = self._check_apply_adaptation(
                eps, b_old, u_k, du, J, L, omega, bcs_du, niter, eps_old
            )
    ...
    def _check_apply_adaptation(
        self, eps, b_old, u_k, du, J, L, omega, bcs_du, niter, eps_old
    ):
    ...
    def _print_iter_info(self, niter, b, du, omega):
        ...
        if 0 == mpiRank:
            print(
                (
                    " Newton iteration {0:1d} (L2 norm) = {1:3.3e}, sup norm = \
{2:3.3e}, omega = {3:g}".format(
                        niter, eps_l2, eps_sup, omega
                    )
                )
            )
...
class m_ncgSolver(mSolver):
    def __init__(
        ...
        self._energy_i = np.zeros(100 * self._N)
    ...
    def f(self, x, *args):
        self._u.vector()[self._free_i] = x
        # self._u.vector().set_local(x.get_local(), self._free_i)
        self._u.vector().apply("")

        aPi = assemble(self._Pi)

        if 0 == self._mpiRank and self._niter % self._print_Niter == 0:
            print((" %d %.12f " % (self._niter, aPi)))
            self._energy_i[np.int(self._niter / self._print_Niter)] = aPi
    ...
    def grad_f(self, x, *args):
        self._u.vector()[self._free_i] = x
        # self._u.vector().set_local(x.get_local(), self._free_i) deprecated
        self._u.vector().apply("")
    ...
    def fixed_centers_solver(self):
        ...
        self._u.vector()[self._free_i] = u_v
    ...
    def grad_f_m(self, x, *args):
        ...
        x_centers_ar = self._gather_center_movement(x)
        self._move_centers(x_centers_ar)
        self._grad_x0.set_local(
            assemble(self._L).get_local()[self._free_i], self._x0_free_i
        )
        self._grad_x0.apply("")
    ...
    def _gather_center_movement(self, x):
        ...
        i2copy = np.array(
            np.linspace(i_start, x.size() - 1, 2 * self._cells_num), dtype="intc"
        )

        if dolfin.__version__ <= "1.6.0":
            x_move_c = PETScVector()
        else:
            x_move_c = PETScVector(MPI.comm_self)
    ...
    def free_centers_solver(self):
        if self._problem.get_k() is None:
            raise ValueError(
                "k is undefined, cell movement is supported only" "for k>0"
            )
        ...
        u_v, warnflag = fmin_cg(
            self.f_m, self._x0, fprime=self.grad_f_m, return_all=True
        )
        # self._niterInfo.updateN(self._niter)
        self._u.vector().set_local(u_v.get_local(), self._free_i)
        self._u.vector().apply("")
    ...
    def _init_cells_centers(self):
        ...
        if mpiRank == mpiSize - 1:
            ...
            i2copy = np.array(
                np.linspace(i_start, self._x0.local_size() - 1, 2 * self._cells_num),
                dtype="intc",
            )
    ...
    def _centre_partial_derivatives(self, x):
        ...
        if mpiRank == mpiSize - 1:
            ...
            i2copy = np.array(
                np.linspace(i_start, x.local_size() - 1, 2 * self._cells_num),
                dtype="intc",
            )
    ...
    def _i_centre_derivative_old2(self, i, x, surface_int_val_old):
        ...
        if mpiRank == mpiSize - 1:
            self._update_vector_i_val(x, x.get_local()[i_start + i] - h, i_start + i)
            self._update_vector_i_val(
                x, x.get_local()[i_start + i + 1] + h, i_start + i + 1
            )
    ...
    def _find_free_indices(self):
        ...
        if self._problem.get_k() == None:
            ...
            # else:
            #     warnings.warn('check if initialization boundary conditions are '
        ...
        self._x0_free_i = np.array(
            np.linspace(0, len(self._free_i) - 1, len(self._free_i)), dtype="intc"
        )
        return
    ...
    def _mark_Rectangular_bnds_partially(self, u):
        leftNright_ex = CompiledSubDomain(
            "(near(x[0], 0) || near(x[0], 1) )&& on_boundary"
        )
        upNdown_ex = CompiledSubDomain(
            "( near(x[1], 0) || near(x[1], 1) ) && on_boundary"
        )
    ...
    def _create_search_vector(self):
        ...
        if mpiRank < mpiSize - 1:
            if dolfin.__version__ >= "2017.2.0":
            ...
            if dolfin.__version__ >= "2017.2.0":
                self._x0.init((prev_sum, int(prev_sum + f_i[mpiRank] + i4_centers_mov)))
            else:
                self._x0.init(
                    self._comm,
                    (prev_sum, int(prev_sum + f_i[mpiRank] + i4_centers_mov)),
                )
        self._grad_x0 = self._x0.copy()
        self._grad_x0_c = self._grad_x0.copy()
        self._grad_x0.apply("insert")
        self._grad_x0_c.apply("insert")
        return
    ...
    def _free_indices_num(self):
        ...
        if dolfin.__version__ <= "1.6.0":
            ...
            numOf_free_i = PETScVector(MPI.comm_self)
        free_i_num_local.gather(
            numOf_free_i, np.linspace(0, mpiSize - 1, mpiSize).astype("intc")
        )
    ...
    def _free_indices_num_mpi4py(self):
        ...
        comm2.Allgather(
            [free_i_num_local.get_local().astype("intc"), INT32_T],
            [numOf_free_i, INT32_T],
        )
