...
class mSolver:
    ...
    def save_all_function_info(self, exp_info=None):
        ...
        try:
            ...
            if self._mpiRank == 0:
                np.save(results_path +'energy_i', self._energy_i[0: np.int(self._niter/self._print_Niter) +1] )
        except:
...
class m_ncgSolver(mSolver):
    def __init__(self, problem, res_path = 'results/', solver = 'fixed_centers',
                 save_run = False, full_output = False, print_Niter = 100):
        mSolver.__init__(self, problem, res_path, save_run = save_run)
...
#         self._niterInfo.updateN(self._niter)
    ...
    def _find_free_indices(self):
        ...
        if self._problem.get_k() == None:
            ...
            self._apply_exterior_bcs(self._u)# It is assumed no boundary conditions for the interior part
    ...
    # It is assumed no boundary conditions for the interior part
    def _apply_exterior_bcs(self, u):
        domain = self._problem.domain

        if domain.has_bc():
            if domain.__class__.__name__ == 'RectangularDomain': 
                bcs = domain.rec_impose_bc(self._problem.V)
                for bc in bcs:
                    bc.apply(u.vector())
            else:
