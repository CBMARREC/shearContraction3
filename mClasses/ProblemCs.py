...
class Problem2D:
    def __init__(self, psi_name=None, domain=None, el_order=None, a=100, b=0.01):
    ...
    def _exp_coef_init(self, psi_name):
        if (
            psi_name is "Rosakis_Grekas3"
            or psi_name is "Rosakis_Grekas5"
    ...
    def parameters_info(self):
        ...
        if self._relax_parameters is not None:
            s += "<br> (b, c) =(%g, %g)" % (
                self._relax_parameters[0],
                self._relax_parameters[1],
            )
    ...
    def Rosakis_exp(self, Ic, J):
        ...
        self._exp_b = 0.01
        return 1.0 / 96 * (
            5 * Ic ** 3 - 9 * Ic ** 2 - 12 * Ic * J ** 2 + 12 * J ** 2 + 8
    ...
    def Grekas04(self, Ic, J):
        ...
        return (
            1.0
            / 48.0
            * (k_t * (5 * Ic ** 3 - 12 * Ic * J ** 2 - 9 * Ic ** 2 + 12 * J ** 2 + 8))
            + exp(self._exp_a * (self._exp_b - J))
            - a / 16.0 * (3 * Ic ** 2 - 4 * J ** 2)
            + a / 2.0 * Ic
            + a / 2
    ...
    def R_G04(self, Ic, J):
        ...
        k_t = 1
        l1, l2 = sqrt(0.5 * (Ic - sqrt(Ic ** 2 - 4 * J ** 2))), sqrt(
            0.5 * (Ic + sqrt(Ic ** 2 - 4 * J ** 2))
        )
        return (
            1.0
            / 48.0
            * (k_t * (5 * Ic ** 3 - 12 * Ic * J ** 2 - 9 * Ic ** 2 + 12 * J ** 2 + 8))
            ...
            - a
            + exp(50 * (0.01 - J))
            + 20 * exp(self._exp_a * (self._exp_b - (l1 + l2)))
        )
    ...
    def R_G(self, Ic, J):
        ...
        return 1.0 / 48.0 * (8 - 24 * a + g5 - (a + 1) * g3 + a * g1) + exp(
            self._exp_a * (self._exp_b - J)
        )
    ...
    def Grekas3(self, Ic, J):
        ...
        a = self._en_alpha
        return 0.5 * (
            1 / 48.0 * (5 * Ic ** 3 - 9 * Ic ** 2 - 12 * Ic * J ** 2 + 12 * J ** 2 + 8)
            - a / 16.0 * (3 * Ic ** 2 - 4 * J ** 2)
            + a / 2.0 * Ic
            - a / 2.0
        ) + exp(self._exp_a * (self._exp_b - J))
    ...
    def memory_relax_tension(self, Ic, J):
        ...
        self._en_alpha, beta, c = (
            0.365,
            self._relax_parameters[0] ** 2,
            self._relax_parameters[1],
        )
        ...
        l9 = (
            63 * Ic ** 5.0 - 280 * Ic ** 3.0 * J ** 2 + 240 * Ic * J ** 4 - 256
        ) / 2560.0
        l7 = (
            35 * Ic ** 4.0
            - 120 * (Ic ** 2.0 - 2 * J ** 2.0) * J ** 2
            - 192 * J ** 4.0
            - 128
        ) / 1024.0
        ...
        return (
            l9
            - (1 + a + 2 * b) * l7
            + (a + 2 * b + 2 * a * b + b ** 2 + c) * l5
            - (2 * a * b + b ** 2 + a * b ** 2 + c * (a + 1)) * l3
            + (a * b ** 2 + a * c) * l
            + exp(self._exp_a * (self._exp_b - J))
        )
    ...
    def l1_minus1(self, Ic, J):
        ...
        return (
            ...
            * (
                ...
                + 3.0
                * (Ic - 2.0 * J2)
                / (10.0 * sqrt(Ic + 2 * J2) + sqrt(Ic + 14.0 * J2))
            )
            + exp(self._exp_a * (self._exp_b - J))
        )
    ...
    def Rosakis_2019_2(self, Ic, J):
        ...
        return (
            ...
            + Ic / J2 ** 3
            - 4 / J2
            + J2 * (-0.1232 + 1.49182 * exp(self._exp_a / J2))
        )

    def Rosakis_2019_3(self, Ic, J):
        self._exp_a, self._exp_b = -80, 0.0
        J2 = (J ** 2 + 0.0001) ** 0.5
        return 2.0 + Ic / J2 ** 3 - 4 / J2
    ...
    def user_w(self, Ic, J):
        ...
        try:
            ...
            raise TypeError(
                "The elastic energy %s is not a function of Ic and J or"
    ...
    def _set_traction_forces(self):
        ...
        for msubdomain in msubdomains_list:
            if msubdomain.has_traction_bc():
                self._add_traction(
                    msubdomain.get_Tn(), msubdomain, domain.get_boundary_parts()
                )
    ...
    def _add_traction(self, Tn, msubdomain, boundary_parts):
        ...
        try:
            self._tractionPi += dot(T, self._u) * ds(
                msubdomain.get_boundary_partNum(), subdomain_data=boundary_parts
            )
        except:
    ...
    def _check_FunctionSpace(self, V):
        if V.__class__.__name__ != "VectorFunctionSpace" or V == None:
            raise TypeError(
                "In %s there is not a VectorFunctionSpace\
            "
                % self.__class__
            )
...
class mNonlinearVariationalProblem(Problem2D):
    ...
    def add_higher_gradients(self, epsilon=1, alpha=8):
        ...
        try:
            ...
            h = 2 * Circumradius(mesh)  # Previously: CellSize(mesh)
        h_avg = (h("+") + h("-")) / 2.0
        n = FacetNormal(mesh)
    ...
    def _update_hg_variatinal_form(self, h_avg, n):
        epsilon2 = Constant(self._epsilon ** 2.0)
...
class mEnergyMinimizationProblem(Problem2D):
    ...
    def _update_hg_energy(self, h_avg, n):
        ...
        self.Pi += epsilon2 * (
            Constant(0.5) * GGu[i, j, k] * GGu[i, j, k] * dx
    ...
    def _add_surface_integrals(self):
        ...
        for msubdomain in msubdomains_list:
            if msubdomain.has_bc():
                self._add_surface_integral(
                    u0, msubdomain, domain.get_boundary_parts(), x_val
                )
                b0, b1 = self._surface_integral_derivative(
                    u0, msubdomain, domain.get_boundary_parts(), x_val
                )
                der_x, der_y = (
                    Constant(self._k * 0.5) * b0,
                    Constant(self._k * 0.5) * b1,
                )
        ...
        try:
            if self._cell_model == "linear_spring":
                self.boundPi += (
                    sqrt(
                        inner(
                            self._u + x - (center_i + d), self._u + x - (center_i + d)
                        )
                    )
                    - (rho - u0_val)
                ) ** 2 * ds(
    ...
    def _surface_integral_derivative(self, u0, msubdomain, boundary_parts, x):
        ...
        try:
            if self._cell_model == "linear_spring":
                ...
                const_part0 = sqrt(
                    inner(self._u + x - (center_i + d), self._u + x - (center_i + d))
                )
                const_part1 = Constant(2) * (const_part0 - (rho - u0_val))
                b0 = (
                    -const_part1
                    / const_part0
                    * (self._u[0] + x[0] - (center_i[0] + d[0]))
                    * ds(
                        msubdomain.get_boundary_partNum(), subdomain_data=boundary_parts
                    )
                )
    ...
    def _update_derivatives(self):
        ...
        for msubdomain in msubdomains_list:
            if msubdomain.has_bc():
                self._DPi0 += self._add_der_surface_integral(
                    self._u1, u01, w, msubdomain, boundary_parts
                )
                self._DPi1 += self._add_der_surface_integral(
                    self._u2, u02, w, msubdomain, boundary_parts
                )
        if self.domain.has_bc():
            self._DPi0 += self._add_der_surface_integral(
                self._u1, u01, w, self.domain, boundary_parts
            )
            self._DPi1 += self._add_der_surface_integral(
                self._u2, u02, w, self.domain, boundary_parts
            )
    ...
    def _add_der_surface_integral(self, u, u_b, w, msubDomain, boundary_parts):
        ...
        return (
            2
            * (dot(u, w) - dot(u_b, w))
            * ds(msubDomain.get_boundary_partNum(), subdomain_data=boundary_parts)
        )
