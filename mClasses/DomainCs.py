...
class Domain:
    def __init__(self, u0=None, Tn=None):
        ...
        if u0 is not None and Tn is not None:
            raise ValueError(
                "Both Dirichlet and Force boundary conditions " "have been given"
            )
    ...
    def check_domain(self):
        if self._domain == None:
            raise NotImplementedError(
                "no domain definition for %s" % self.__class__.__name__
            )
        return

    def _check_mesh_val(self):
        if self._mesh == None:
            raise NotImplementedError("undefined mesh for %s" % self.__class__.__name__)
        return
    ...
    def refine_mesh(self):
        mesh = self._mesh
        marker = MeshFunction('size_t',mesh,mesh.topology().dim()) # Previously: CellFunction("bool", mesh, True)
    ...
    def band_refinement(self):
        ...
        cell_markers = MeshFunction("size_t", self._mesh, self._mesh.topology().dim())
    ...
    def _apply_band_refinement(self, myDomain1, myDomain2, cell_markers):
        ...
        if (
            myDomain1.__class__.__name__ == "RectangularDomain"
            or myDomain2.__class__.__name__ == "RectangularDomain"
        ):
    ...
    def rect_band_refinement(self, min_x, min_y, max_x, max_y):
        cell_markers = MeshFunction('size_t', self._mesh, self._mesh.topology().dim()) # CellFunction("bool", self._mesh, True)
        self._init_cell_markers(cell_markers)
...
class CircularDomain(Domain):
    def __init__(self, rho=10, x=0, y=0, u0=None, Tn=None, bound_res=50):
        ...
        try:
            ...
            self._domain = mshr.Circle(
                Point(x, y), rho, bound_res
            )  # version 2018.1.0 or later
...
class RectangularDomain(Domain):
    def __init__(self, x1=0, y1=0, x2=1, y2=1, sides_bc=None, isMeshUniform=False):
        ...
        try:
            ...
            self._domain = mshr.Rectangle(
                Point(x1, y1), Point(x2, y2)
            )  # version 2018.1.0 or latter
        self._tol = 1e-7
    ...
    def create_bc2(self, V, lambda1=1, lambda2=1):
        ...
        if dolfin.__version__ >= "2016.2.0":
            ...
            u0 = Expression(
                ("lambda1*x[0]", "lambda2*x[1]"), lambda1=lambda1, lambda2=lambda2
            )
    ...
    def _impose_bc(self, V):
        if self._sides_bc == None:
            return None
        bcs = []
        for u0_l in self._sides_bc:
            bc = DirichletBC(V, u0_l[0], u0_l[1])
            bcs.append(bc)
        return bcs
    ...
    def _impose_bc_old(self, V):
        ...
        if lambda1 == None or lambda2 == None:
            raise NotImplementedError(
                "boundary conditions for one of the boundaries free"
...
class RectangularBoundary(SubDomain):
    ...
    def inside(self, x, on_boundary):
        tol = self.mtol
        onb_rect = (
            between(x[0], (self._mx1, self._mx1 + tol))
